...
##### 3. W2V  ##### 
  ...
  Cosine_Simi.df <- word2vec_similarity(embedding, embedding[row.names(embedding)==KeyWord,]
                                        , top_n = +Inf, type = c("dot", "cosine"))
  colnames(Cosine_Simi.df)[3] <- "Cos_Similarity"
  Cosine_Simi.df <- arrange(Cosine_Simi.df,Cos_Similarity)
...
##### 4. Combine BM25 and Similarity #####
  ...
  # Cosine_Simi
  colnames(Cosine_Simi.df)[1] <- "word"
  PMID_BM25_W2V.df <- left_join(PMID_BM25_W2V.df,Cosine_Simi.df[c(1,3)],by = "word")
  PMID_BM25_W2V.df$Cos_Similarity[is.na(PMID_BM25_W2V.df$Cos_Similarity)] <- 0
  PMID_BM25_W2V.df <- PMID_BM25_W2V.df %>% mutate(CosScore = bm25*abs(CSP*Cos_Similarity))
  
  summary(PMID_BM25_W2V.df$CosScore)
##### 5. Visualization #####
...
##### 6. Rank the PMID #####
  ...
  PMID_BM25_W2V.df <- PMID_BM25_W2V.df %>% 
                      group_by(PMID) %>% 
                      mutate(PMIDCosScore=sum(CosScore)) %>%
                      arrange(desc(PMIDCosScore))
  PMID_CosRank.df <- PMID_BM25_W2V.df %>% 
                  group_by(PMID) %>% 
                  slice(which.max(CosScore)) %>%
                  arrange(desc(PMIDCosScore))
  densityplot( ~ PMIDCosScore ,      
               data=PMID_CosRank.df
  ) 
